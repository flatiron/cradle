...
cradle.Connection = function Connection(/* variable args */) {
    ...
    this.options.retry      = this.options.retry      || cradle.retry;
...
//      content
//
//      By default, the request will be retried once. Set the `retry` option to
...
cradle.Connection.prototype.rawRequest = function (method, path, options, data, headers) {
    ...
    this._rawRequest(promise, {
        ...
        path:    path,
        headers: headers
    }, data, null, this.options.retry);

    return promise;
}

cradle.Connection.prototype._rawRequest = function (promise, options, data, chunks, retry) {
    var request = this.socket.request(options);

    if (chunks) {
        chunks.forEach(function (chunk) { request.write(chunk) });
        request.end();
    } else if (data) {
        if (data.on) {
            chunks = [];
    ...
    request.on('error', function (err) {
        ...
        if (err.message === 'socket hang up') {
            err.message = 'ECONNRESET, Client reset by peer';
            err.code = 'ECONNRESET';
        }
        ...
        {
            promise.emit('retry');
            return that._rawRequest(promise, options, data, chunks, retry);
        }
