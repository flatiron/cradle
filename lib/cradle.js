        ...
        insert: function (/* [id], doc, ... */) {
            ...
            if (typeof(args.first) === 'string') {
                ...
                if (/^_design\/\w+$/.test(id)) {
                    ...
                    doc.views = {};
                    doc.updates = {};
                    Object.keys(args.last).forEach(function(k) {
                        if(args.last.hasOwnProperty(k)) {
                            if(typeof(args.last[k]) === 'object' &&
                              (args.last[k].map ||
                              args.last[k].reduce ||
                              args.last[k].rereduce)) {
                                doc.views[k] = args.last[k];
                            } else if(typeof(args.last[k]) === 'function') {
                                doc.updates[k] = args.last[k];
                            }
                        }
                    });
                    if(!doc.views)
