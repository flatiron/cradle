...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        //
        // The database bulk save cache
        //
        bulkCache: new(cradle.Cache)(cradle.merge({}, that.options, {cache: true, cacheSize: 0})),
        ...
        get: function (id, rev) {
            ...
            if (Array.isArray(id)) { // Bulk GET
                ...
                } else if (this.bulkCache.has(id)) {
                    return args.callback(null, this.bulkCache.get(id));
        ...
        save: function (/* [id], [rev_or_bulk], [bulk], doc | [doc, ...] */) {
            var args = new(Args)(arguments),
                array = args.all.slice(0), doc, id, rev, bulk;
            ...
            this._save(id, rev, doc, bulk, args.callback);
        },
       _save: function (id, rev, doc, bulk, callback) {
            ...
            if (Array.isArray(doc)) {
                ...
                this.query('POST', '/_bulk_docs', {}, document, function(err, res) {
                    ...
                    async.filter(res, function (doc, predicate) { predicate(doc.error === 'conflict'); },
                    function (conflictedDocs) {
                        // Return if no docs conflicted
                        if (conflictedDocs.length == 0) return callback && callback(err, res);

                        // Grab latest revision for all conflicted docs
                        async.map(conflictedDocs, function(doc, complete) {
                            var index = res.indexOf(doc);
                            that.head(doc.id, function (e, headers) {
                                // Remove conflicted doc from origin result
                                res.splice(index, 1);
                                document.docs[index]._rev = headers['etag'].slice(1, -1);
                                complete(null, document.docs[index]);
                            });
                        },
                        function(err, docs) {
                            // Save conflicted docs again and concatenate with already saved results
                            that.save(docs, function (err, conflictedRes) {
                                async.reject(res, function(doc, predicate) { predicate(doc.error === 'conflict') },
                                function (res) { callback && callback(err, res.concat(conflictedRes)); });
                            });
                      });
                    });
                });
                ...
                if (id) {
                    ...
                    if (document._rev) {
                        this.put(id, document, bulk, callback);
                    } else if (this.cache.has(id)) {
        ...
        flush: function (callback) {
            var options = connection.options,
                cache = this.cache,
                bulkCache = this.bulkCache,
                bulkStore = bulkCache.store,
                keys = Object.keys(bulkStore),
                that = this;

            if (keys.length == 0) {
                return process.nextTick(function () {
                    callback && callback(null, new(Array));
                });
            }
            ...
            for (var i = 0; i < keys.length; i++) {
                var id = keys[i];
                docs.push(cradle.merge({_id: id}, bulkStore[id].document));
            }

            this.save(docs, function (err, res) {
                if (err) return callback && callback(err, res);
                ...
                for (var i = 0; i < res.length; i++) {
                    ...
                    // Update revision and put it in cache, purge it from bulk cache
                    item.document._rev = rev;
                    cache.save(id, item.document);
                    bulkCache.purge(id);
                }
       ...
       _merge: function (id, doc, callback) {
        ...
        //
        put: function (id, doc, bulk, callback) {
            var options = connection.options,
                cache = this.cache,
                bulkCache = this.bulkCache;
            ...
            // Defer PUT but write document to bulk cache, if bulk
            if (bulk) {
              bulkCache.save(id, doc);

              // Automatically flush if bulkSize is set and number of documents is bigger
              if (options.bulkSize > 0 && bulkCache.keys >= options.bulkSize) {
                this.flush(callback);
              } else {
                process.nextTick(function () {
                  callback && callback(null, {ok: true, bulk: true, id: id});
                });
              }
            } else {
