    ...
    return {
       ...
       _merge: function (id, doc, callback) {
            ...
            if (callback) {
                ...
                that.rawRequest('GET', [name, '_changes'].join('/'), options).on('response', function (res) {
                    ...
                    res.on('data', function (data) { data.trim() && response.emit('data', JSON.parse(data)) })
                       .on('end',  function ()     { response.emit('end') });
                });
                return promise;
            }
        },
        ...
        temporaryView: function(doc, callback) {
          console.log('WARNING: Only use `temporaryView` for testing, because it'
                     + ' will be EXXTREEMELY slow in production. Cheers!');
          this.query('POST', '_temp_view', undefined, doc, callback ? callback : undefined);
        }
    }
