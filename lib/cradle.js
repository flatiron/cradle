...
cradle.Connection.prototype.request = function (method, path, options, data, headers) {
    ...
    request.addListener('response', function (res) {
        ...
        // Don't wait for data events if it's a HEAD request
        if (method === 'HEAD') {
            promise.emit("done", null, res.statusCode, res.headers);
            promise.emit("success", res.statusCode, res.headers);
            return;
        }
...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        head: function (id) {
            ...
            return this.query('HEAD', id, null).addCallback(function (e, status, headers) {
                args.callback(e, headers, status);
            });
        },
...
cradle.Response = function Response(response, json) {
    ...
    if (json.rows) {
        ...
        Object.keys(json).forEach(function (k) {
            obj.__defineGetter__(k, function () { return json[k] });
        });
        obj.__defineGetter__('doc', function () { return json });
    ...
    obj.__proto__['headers' in obj ? '_headers' : 'headers'] =
        mixin({}, response.headers, { status: response.statusCode });

    if (!obj.id  && obj._id)  { obj.__defineGetter__('id', function () { return obj._id }) }
    if (!obj.rev && obj._rev) { obj.__defineGetter__('rev', function () { return obj._rev }) }

    if (Array.isArray(obj)) {
        obj.__proto__.forEach = function (f) {
            for (var i = 0; i < obj.rows.length; i++) {
                if (f.length === 1) {
                    f.call(obj.rows[i], obj.rows[i]);
                } else {
                    f.call(obj.rows[i], obj.rows[i].key, obj.rows[i].value, obj.rows[i].id);
                }
            }
        };
        obj.__proto__.map = function (f) {
            var ary = [];
            if (f.length === 1) {
                this.forEach(function (a) { ary.push(f.apply(this, a)) });
            } else {
    ...
    obj.__proto__.toString = function () { return sys.inspect(this.doc) };
