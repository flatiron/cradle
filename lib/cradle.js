...
cradle.Connection = function Connection(/* variable args */) {
    var args = Array.prototype.slice.call(arguments),
        host, port, remote, auth, secure, options = {};

    if (typeof(args[0]) === 'string') {
        remote = args[0].replace('https?://', '').split(':');
        host = remote[0];
    ...
    this.secure = secure || cradle.secure;
    this.options = cradle.merge({}, cradle.options, options);

    this.socket = http.createClient(this.port, this.host, this.secure);
