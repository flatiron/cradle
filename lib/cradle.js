...
cradle.Connection.prototype.rawRequest = function (method, path, options, data, headers) {
    ...
    if (!headers.host) { headers.host = this.host; }
    if (options) {
...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        saveAttachment: function(doc, attachmentName, contentType, data_or_stream, callback) {
            var req, id, pathname, headers = {}, response, body = '', headers, resHeaders, error, db = this;
            pathname = '/' + [name, doc._id, attachmentName].join('/');
            headers['Content-Type'] = contentType
            req = that.rawRequest('PUT', pathname, {rev: doc._rev}, data_or_stream, headers)
                .addListener('response', function(res){ resHeaders = {status: res.statusCode}; })
                .addListener('data', function(chunk) { body += chunk; })
                .addListener('end', function() { 
                    response = JSON.parse(body);
                    response._headers = resHeaders;

                    if (response._headers.status == 201 && db.cache.store[doc._id]) {
                        cached = db.cache.store[doc._id];
                        cached._rev = response.rev;
                        if (! cached._attachments) {
                            cached._attachments = {};
                        }
...
cradle.Response = function Response(response, json) {
    ...
    if (this.rows) {
        this.__proto__.forEach = function (f) {
            for (var i = 0; i < that.rows.length; i++) {
                f.call(null, that.rows[i].key, that.rows[i].value);
            }
        };
        this.__proto__.map = function (f) { return that.rows.map(f) };
    }
    this.__proto__.toString = function () { return sys.inspect(this.doc) };
};
