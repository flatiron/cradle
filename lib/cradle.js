...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        //
        // The bulk documents queue.
        //
        bulkQueue: new(Array),

        //
        // The documents pushed for bulk saving
        bulkDocs: new(Object),
        ...
        save: function (/* [id], [rev], doc | [doc, ...] */) {
            var that = this, args = new(Args)(arguments),
                array = args.all.slice(0), doc, id, rev, uniq = {};

            // Save bulk docs if no args
            if (args.length === 0) {
                // Save only latest version of documents by filtering in reverse
                this.bulkQueue.reverse();
                doc = new(Array);
                this.bulkQueue.forEach(function (id) {
                    if (!uniq[id]) {
                        uniq[id] = true;
                        doc.push(that.bulkDocs[id])
                    }
                });
                // Restore original order and empty bulk docs and queue
                doc.reverse();
                this.bulkDocs = new(Object);
       ...
       _save: function (id, rev, doc, callback) {
            ...
            var document = {}, that = this, cache = this.cache;
            var conflictedDocs, index;

            // Bulk Insert
            if (Array.isArray(doc)) {
                document.docs = doc;
                if (options.allOrNothing) { document.all_or_nothing = true }
                this.query('POST', '/_bulk_docs', {}, document, function(err, res) {
                    if (err) return callback && callback(err);

                    // Cache saved documents
                    res.forEach(function (res, index) {
                        if (res.id && res.rev) {
                            cache.save(res.id, cradle.merge({}, doc[index], { _rev: res.rev }));
                        }
                    });

                    // Filter conflicted docs
                    conflictedRes = res.filter(function (doc) { return doc.error === 'conflict' });

                    // If no docs conflicted, return
                    if (conflictedRes.length === 0) { return callback && callback(null, res) }

                    // Grab latest revision for all conflicted docs
                    nimble.map(conflictedRes, function (doc, callback) {
                        that.head(doc.id, function (err, headers) {
                            if (err) return callback && callback(err);

                            index = res.indexOf(doc);
                            document.docs[index]._rev = headers['etag'].slice(1, -1);
                            callback(null, document.docs[index]);
                        });
                    }, function (err, conflictedDocs) {
                        if (err) return callback && callback(err);
                        // Save conflicted docs again and concatenate with previous non-conflicted results
                        that.save(conflictedDocs, function (err, conflictedRes) {
                            if (err) return callback && callback(err);
                            // Merge result of conflicted documents into original result by id
                            ids = res.map(function (doc) { return doc.id });
                            conflictedRes.forEach(function (doc) {
                              index = ids.indexOf(doc.id);
                              res[index] = doc;
                            });
       ...
       _merge: function (id, doc, callback) {
        ...
        push: function (/* [id], [rev], doc | [doc, ...] */) {
            var args = new(Args)(arguments),
                array = args.all.slice(0), doc, docs, that = this;

            // Push multiple documents
            if (Array.isArray(args.first)) {
                docs = args.first;

                // Iterate over documents and push them one by one
                docs.forEach(function (doc) {
                    that._push(doc._id, doc._rev, doc);
                });
            } else {
                doc = array.pop();
                id = array.shift() || doc._id;
                rev = array.shift();
                this._push(id, rev, doc);
            }
        },
        _push: function (id, rev, doc) {
            var document = {};
            // Push a single document, with an id (Create or Update)
            if (id) {
                // Design document
                if (/^_design\/(\w|%)+$/.test(id) && !('views' in doc)) {
                    document.language = "javascript";
                ...
                // Set '_id' if id was specified
                id && (document._id = id);

                // Try to set the '_rev' attribute of the document.
                // Attempt to retrieve it from the cache.
                if (document._rev === undefined && this.cache.has(id)) {
                    document._rev = this.cache.get(id)._rev;
                }

                // Store a single document for bulk saving
                this.bulkDocs[id] = (document);

                // Push to Bulk Docs queue
                this.bulkQueue.push(id)
            } else {
              // TODO: Push a single document, without an id (Create)
              console.log('WARN: Bulk saving documents without ids is not supported yet');
            }
        },
