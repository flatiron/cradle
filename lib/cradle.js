...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        saveAttachment: function(doc_or_id, attachmentName, contentType, data_or_stream, callback) {
            var req, rev, id, pathname, headers = {}, response, body = '', headers, resHeaders, error, db = this;
            if (typeof doc_or_id === 'string') { 
                id = doc_or_id;
                var doc =  db.cache.get(id);
                if (doc) { rev = {rev: doc._rev}; }
            }
            else { 
                id = doc_or_id._id;
                if (doc_or_id._rev) {
                    rev = {rev: doc_or_id._rev };
                } else { rev = {} }
            }
            pathname = '/' + [name, id, attachmentName].join('/');
            headers['Content-Type'] = contentType
            req = that.rawRequest('PUT', pathname, rev, data_or_stream, headers)
                ...
                .addListener('end', function() { 
                    ...
                    if (response._headers.status == 201 && db.cache.store[id]) {
