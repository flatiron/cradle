...
function Args(arguments) {
    ...
    this.callback = typeof(this.at(-1)) === 'function' ? this.at(-1)
                                                       : function () {};
}
...
cradle.Connection.prototype.database = function (name) {
    ...
    return {
        ...
        // A wrapper around POST, PUT and _bulk_docs.
        // It handles an optional '_rev', as well as saving '_design' docs.
        // After the document was saved successfully, it's written through the cache,
        // via a callback. The returned '_rev' is also saved.
        save: function (/* [id], [rev], doc, ... */) {
            var id, doc = {}, that = this, args = new(Args)(arguments);

            // PUT a single document, with an id
            if (typeof(args.first) === 'string' && args.length > 1) {
                id = args.first;
                // Design document
                ...
                if (typeof(args.at(1)) === 'string') { doc._rev = args.at(1) }
        ...
        update: function (id, obj) {
            ...
            if (doc) {
                return this.save(id, process.mixin(true, {}, doc, obj), Args.last(arguments));
            } else {
