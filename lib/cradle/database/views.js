...
// Cloudant Full Text Search (cfts)
Database.prototype.cfts = function (path, options, callback) {
    if (typeof options === 'function') {
        callback = options;
        options = {};
    }
    path = path.split('/');
    path = [this.name, '_design', path[0], '_search', path[1]].map(querystring.escape).join('/');
    // http://user.cloudant.com/mydatabase/_design/mydesigndocument/_search/by_myname?q=*%3A*
